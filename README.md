...
- Mindustry's `DesktopLauncher.checkJavaVersion()` requires `arc.util.OS.javaVersionNumber >= 17`, and `OS.<clinit>` parses `System.getProperty("java.version")` into that field. CheerpJ ignores `java.version` overrides (both `javaProperties` and runtime `System.setProperty`) and pins it to `1.8.0_xxx`, so `OS.<clinit>` takes the `startsWith("1.")` branch and hardcodes `javaVersionNumber = 8`. `build.sh` patches that single `bipush 8` to `bipush 25` in `arc/util/OS.class` and ships the result via the override classpath.
